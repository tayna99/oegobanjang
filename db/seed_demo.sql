...
--
-- 주의
--  * 이 파일은 **seed_reference.sql 뒤에** 로드해야 한다(schema → reference → demo). 전역
--    A/B 근거·document_requirements는 seed_reference.sql이 넣는다 — 여기 아래 case_citations가
--    참조하는 cit_001~011은 그 파일에서 온다. db/load.py --with-demo 또는 validate.py가 순서 보장.
...
-- 이대표(공동대표 owner)·최감사(viewer)는 mock COMPANY_MEMBERS엔 있으나 시드엔 없어 real 모드
-- RBAC 데모(공동대표 배너·viewer 라우트 가드)가 재현되지 않던 것을 보강한다([데모 보강]).
-- 이대표는 승인 가능하므로 pin_hash 유지(동일 데모 PIN '1234'), 최감사는 승인 불가라 NULL.
INSERT INTO users (id, phone, name, terms_agreed_at, pin_hash) VALUES
  ...
  ('usr_owner', '010-0000-0003', '김대표', '2026-06-01T09:20:00Z',
   'f6069dbc9e0c0f3a844b39d21e6664b359cbdd2f1cf32b22f8afbb317f5aa86a'),
  ('usr_lee',   '010-0000-0004', '이대표', '2026-06-01T09:30:00Z',
   'f6069dbc9e0c0f3a844b39d21e6664b359cbdd2f1cf32b22f8afbb317f5aa86a'),
  ('usr_choi',  '010-0000-0005', '최감사', '2026-06-01T09:40:00Z', NULL);
...
INSERT INTO memberships (id, company_id, user_id, role, status) VALUES
  ...
  ('mem_lee',   'cmp_greenfood', 'usr_lee',   'owner',   'active'),
  ('mem_choi',  'cmp_greenfood', 'usr_choi',  'viewer',  'active');
...
-- 4.4 근거 라이브러리 — 회사 스코프 E등급 내부 근거만 (src/mocks/citations.ts의 내부 항목)
-- 전역 A/B등급 근거(cit_001~011)와 전역 document_requirements는 db/seed_reference.sql로
-- 승격됐다(모든 환경 필수). 이 파일은 seed_reference.sql 뒤에 로드되므로, 아래 케이스·
-- case_citations가 참조하는 cit_001/cit_002/cit_003/cit_004/cit_007/cit_009는 이미 존재한다.
-- cit_014/cit_021은 내부 기준(김담당) → company 스코프라 전역이 아니고 여기 남는다.
...
-- 스레드 발신 감사 기록([데모 보강] — src/mocks/threads.ts의 tran/bayar 발신 버블 evidenceRef).
-- 스레드(th_tran/th_bayar)의 system 발신 메시지와 쌍이며, 발송 실행 이력을 표현한다.
INSERT INTO evidence_events (id, company_id, event_no, type, at, case_id, action_id,
                             approval_id, run_id, actor_type, actor_user_id, actor_display,
                             summary, input_hash) VALUES
  ('ev_4720', 'cmp_greenfood', 4720, 'dispatch_executed', '2026-07-03T15:40:00Z',
   'cs_batbayar', NULL, NULL, NULL, 'user', 'usr_kim', '김담당',
   'Batbayar E. · 건강검진 일정 확인 SMS 발송', 'sha256:4b20…d7a1'),
  ('ev_4742', 'cmp_greenfood', 4742, 'dispatch_executed', '2026-07-01T09:20:00Z',
   'cs_tran', NULL, NULL, NULL, 'user', 'usr_kim', '김담당',
   'Tran Thi H. · 계약 종료일 안내·서류 확인 Zalo 발송', 'sha256:47a2…e0c8');
...
-- 스레드 3종(src/mocks/threads.ts THREADS 패리티) — mock은 nguyen(빈 초안 스레드)·tran(응답
-- 도착)·bayar(SMS 발신)를 갖는데 시드엔 th_tran만 있었다. th_nguyen·th_bayar 보강([데모 보강]).
-- 발신 메시지는 direction='system'(스키마가 outbound를 금지 — 우리가 보낸 것은 system).
INSERT INTO threads (id, company_id, worker_id, channel, last_message_at) VALUES
  ('th_nguyen', 'cmp_greenfood', 'wrk_nguyen',   'zalo', NULL),                    -- 초안 단계, 발송된 메시지 없음
  ('th_tran',   'cmp_greenfood', 'wrk_tran',     'zalo', '2026-07-10T10:12:00Z'),
  ('th_bayar',  'cmp_greenfood', 'wrk_batbayar', 'sms',  '2026-07-03T15:40:00Z');

INSERT INTO thread_messages (id, thread_id, company_id, direction, lang,
                             body_original, body_ko, received_at) VALUES
  -- th_tran 발신(계약 종료일 안내 · 서류 확인 요청) — evidence #4742와 쌍
  ('tm_tran_out', 'th_tran', 'cmp_greenfood', 'system', 'vi',
   'Chào anh Tran, hợp đồng của anh sẽ kết thúc vào ngày 18.8. Vui lòng xác nhận bản sao hợp đồng lao động và hộ chiếu.',
   '계약 종료일(8.18) 안내 · 표준근로계약서·여권 사본 확인 요청', '2026-07-01T09:20:00Z'),
  ...
  -- th_bayar 발신(건강검진 일정 확인 SMS) — evidence #4720과 쌍
  ('tm_bayar_out', 'th_bayar', 'cmp_greenfood', 'system', 'mn',
   'Сайн байна уу Bayar, эрүүл мэндийн үзлэгийн товыг баталгаажуулна уу.',
   '건강검진 일정 확인 요청', '2026-07-03T15:40:00Z');
