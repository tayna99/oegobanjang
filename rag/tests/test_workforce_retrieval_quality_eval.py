...
from oe_rag import evaluate_workforce
from oe_rag.config import RAG_ROOT
from oe_rag.evaluate_workforce import DEFAULT_DATASET_PATH

PLAN_DOC_PATH = RAG_ROOT / "docs" / "WORKFORCE_RETRIEVAL_QUALITY_EVAL.md"


def test_workforce_retrieval_quality_dataset_contract() -> None:
    rows = list(csv.DictReader(DEFAULT_DATASET_PATH.open("r", encoding="utf-8-sig")))
    required_columns = {
        "test_id",
        "question",
        "intent",
...
def test_compute_case_result_calculates_hits_and_mrr() -> None:
    result = evaluate_workforce.compute_case_result(
        case={
            "test_id": "T999",
            "question": "테스트 질문",
