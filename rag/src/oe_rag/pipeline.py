"""문서 수집→파싱/전처리→청킹→메타 검증→JSONL 산출 파이프라인.

legacy scripts/ingest_rag_docs.py의 충실 이식. 차이점:
- 경로는 oe_rag.config 기준 (data-pipeline/ 상대 레이아웃은 legacy와 동일하게 유지 —
  .md/.txt 원문의 source_id가 상대 경로 sha1로 파생되기 때문)
- chunk_id에 내용 해시 접미사(`_{hash8}`)를 추가해 멱등 upsert를 지원
  (legacy: `{source_id}_chunk_{index:04d}` → 신규: `{source_id}_chunk_{index:04d}_{hash8}`)
"""
...
from .chunking import chunk_document, chunk_hash
from .config import CHUNKS_DIR, PROCESSED_DIR, RAG_ROOT, RAW_DIR, SEED_DIR, env_flag
from .embeddings import deterministic_embedding
from .evaluate import evaluate_retrieval
from .ingest.raw_ingest import RawIngestor, build_ingestion_report
from .ingest.workforce_metadata import (
...
def make_chunks_from_record(record: dict[str, Any], source_path: str | None = None) -> list[dict[str, Any]]:
    ...
    for index, chunk_text in enumerate(chunks):
        chunk_id = f"{metadata['source_id']}_chunk_{index:04d}_{chunk_hash(chunk_text)}"
...
def load_seed_documents(include_demo_seed: bool | None = None) -> list[dict[str, Any]]:
    if include_demo_seed is None:
        include_demo_seed = env_flag("DAILY_BRIEFING_INCLUDE_DEMO_SEED")
...
# legacy 이름은 *_chroma_records.jsonl — 스토어 중립 이름으로 변경 (내용 계약은 동일)
WORKFORCE_COLLECTION_FILE_NAMES = {
...
def run_ingest(
    *,
    ...
    dry_run: bool = False,
    report: bool = False,
    include_demo_seed: bool | None = None,
    ...
    all_chunks, raw_report = build_all_chunks(include_demo_seed=include_demo_seed)
    ...
    for file_name in EXPECTED_CHUNK_FILES:
    ...
    for collection, file_name in WORKFORCE_COLLECTION_FILE_NAMES.items():
        path = CHUNKS_DIR / file_name
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--include-demo-seed",
        action="store_true",
        default=env_flag("DAILY_BRIEFING_INCLUDE_DEMO_SEED"),
    ...
    return run_ingest(
        ...
        report=args.report,
        include_demo_seed=args.include_demo_seed,
