"""워크포스 런타임 검색 품질 평가 — legacy scripts/evaluate_workforce_retrieval.py 이식.

게이트: Hit@1 >= 0.60, Hit@3 >= 0.80, Hit@5 >= 0.90, MRR >= 0.65,
Safety Fail = 0, official_misuse_count = 0.
"""
...
from .config import CHUNKS_DIR, DATASETS_DIR, REPORTS_DIR
from .retriever import retrieve_workforce_runtime_materials

DEFAULT_DATASET_PATH = DATASETS_DIR / "workforce_retrieval_quality_cases.csv"
DEFAULT_REPORT_CSV = REPORTS_DIR / "workforce_retrieval_quality_latest.csv"
DEFAULT_REPORT_JSON = REPORTS_DIR / "workforce_retrieval_quality_latest.json"

COLLECTION_RECORD_FILES = (
    "workforce_official_vector_records.jsonl",
    "workforce_templates_vector_records.jsonl",
)
...
def load_known_sources(chunks_dir: Path = CHUNKS_DIR) -> dict[str, dict[str, Any]]:
    sources: dict[str, dict[str, Any]] = {}
    for file_name in COLLECTION_RECORD_FILES:
...
def gates_passed(
    ...
    *,
    min_hit_at_1: float = 0.60,
    min_hit_at_3: float = 0.80,
    min_hit_at_5: float = 0.90,
    min_mrr: float = 0.65,
) -> bool:
    return (
        summary["hit_at_1"] >= min_hit_at_1
        and summary["hit_at_3"] >= min_hit_at_3
        and summary["hit_at_5"] >= min_hit_at_5
