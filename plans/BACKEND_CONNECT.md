...
rag/ (FastAPI :8100, py3.13, pgvector :55433)   ← 내부 전용(backend만 호출)
  │  /retrieve · /agent/run · /graph/run 은 backend 전용 내부 API (서비스 인증·내부망)
  ▼
...
- FastAPI 앱 추가 (`uv run uvicorn oe_rag.api:app --port 8100`):
  ...
  - `POST /agent/run`/`POST /graph/run`은 backend만 쓰는 내부 SSE 계약이다. 브라우저가 직접 호출하지 않으며, 배포에서는 내부망과 서비스 인증으로 닫는다.
...
- RagCitation은 모델 출력 metadata를 신뢰하지 않는다. rag의 deterministic retrieval이 만든 candidate catalog에서 허용 `source_id`를 확인하고 title·grade·URL을 재주입한 뒤에만 `citations` upsert한다. A/B/E만 허용하며, unknown source·D/F·타사 internal citation은 거부한다. E는 catalog의 현재 회사 scope가 필수이고, 전역 PK `citations.id`가 다른 scope와 충돌하면 기존 행을 갱신하거나 재귀속하지 않고 거부한다.
...
- `POST /api/v1/runs/stream` `{company_id, message}` → SSE:
  ...
  ④ backend 입구에서 PII 스크럽 후에만 `runs.goal_text`·RAG·SSE·evidence를 처리하고, client `thread_id`는 거부하며 run/case 기반 서버 key만 RAG에 전달 ⑤ `structured`의 citation source는 candidate catalog로 검증·정본화한 뒤 `upsert_citations()` ⑥ `approval.required`면
...
- DoD: `respx`로 rag `/intent`·`/graph/run`을 목킹한 통합 테스트(fake rag 서버 원칙),
  ...
  테넌트 인가(멤버십 없는 회사 403), client thread key 422, PII 원문이 goal/evidence/SSE/RAG 요청에 남지 않음, 모델 citation의 title/grade 승격·unknown source가 정본 catalog를 바꾸지 않음.
...
- langgraph-checkpoint-postgres는 활성 membership·same-company case에서 파생한 opaque server thread key만 받게 하고, PII 스크럽·TTL·파기·사용자 삭제 정책과 회귀 테스트가 준비된 뒤 도입. Playwright 크롤러 `[crawl]` extra로 코퍼스 갱신
...
- 가드레일 회귀: D/F/후보 밖 citation 비노출, rag_retrieved와 `runs.goal_text`·SSE·checkpoint에 원문 PII 없음, client thread key 거부, 발송·제출 자동 실행 금지
