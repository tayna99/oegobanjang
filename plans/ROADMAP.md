...
### R5.1 — 행정사 화이트라벨 v1 백엔드 착지(2026-07-20)

이 문서의 데이터 모델·API 설계를 실제 백엔드로 구현. **법무 미확정 상태는 그대로다** —
위탁(§26) vs 제3자 제공(§17) 법적 성격 분류는 여전히 미확정이며(§5.5·§9·§10), 이번
착지는 spec이 명시한 **잠정 가정(결정 A/B/C의 default 방향)을 그대로 구현**한 것이지
그 법적 전제를 확정한 것이 아니다. 분류가 뒤집히면 `ExpertGrant`가 tenant 단위 →
tenant+worker 단위로 커지는 스키마 변경이 여전히 필요하다.

- 신규 테이블 7개(Alembic `0005_r5_1_expert_whitelabel`, `db/schema.sql` §4.8-1):
  `expert_accounts`/`expert_office_members`/`expert_grants`/`package_view_log`/
  `pii_field_policies`/`expert_login_otps`/`expert_sessions`. `handoff_packages`에
  `expert_account_id`(nullable, v0 하위호환) 추가.
- API(`/api/v1/expert/*`): 위탁 발급/승인확인/철회/목록(owner·manager 내부 세션), 사무소
  구성원 CRUD(사무소 자체 세션, isOfficeAdmin 오너십), email+OTP 로그인, 패키지 조회
  (`GET /api/v1/expert/packages/{id}` — spec §4.2 3중 체크: tenant scope + 사무소 일치 +
  케이스 승인 게이트, 실패 시 전부 동일 404) + `PackageViewLog` 기록. 무기한 위탁은
  DB CHECK(`until_date > from_date` + NOT NULL)로 이중 차단(API 레벨 검증 + DB 레벨 강제).
- **spec §3.1 인증 모델 편차(의도적)**: legacy JWT 계보 대신 이 저장소의 기존 phone+OTP+
  불투명 세션 패턴(`login_otps`/`sessions`)을 email 축으로 특수화했다(`expert_login_otps`/
  `expert_sessions`) — 이 코드베이스 어디에도 비밀번호 해시 인프라가 없어 spec의
  email+비밀번호(PBKDF2)를 그대로 도입하면 인증 메커니즘이 두 갈래로 갈라진다. 보안
  속성(서명·exp 검증·세션 폐기)은 동등하게 충족한다 — 상세 근거는
  `docs/DB_SCHEMA.md` §4.8-1.
- **의도적으로 미룬 것**: (1) `ExpertGrant` 5단계 상태 전이의 DB 트리거 상태머신(서비스
  계층에서만 강제 — `handoff_packages`/`drafts`와 달리), (2) `PiiFieldPolicy`를 실제로
  소비해 패키지 콘텐츠를 마스킹 렌더링하는 API(콘텐츠 자체가 아직 서버에 없음 — R2.6과
  동일 경계), (3) §2.4.1 자유 텍스트 PII 정규식 스캔 방어(재사용 대상 `contains_pii()`는
  이미 존재하나 이번 R5.1이 새로 쓰는 evidence/로그엔 자유 텍스트 PII 필드가 없어 당장
  불필요), (4) **v0(mock, `src/mocks/expert.ts` 등) → v1 프론트 재배선은 시도하지 않음**
  — 이번 mission은 백엔드 인프라가 범위이고, 프론트 재배선은 스코프상 stretch goal로
  명시돼 있었다(561+ 기존 mock-mode 테스트를 건드리지 않기 위해 착수하지 않음 — 프론트
  파일은 전혀 수정하지 않았다).
- 검증: `db/validate.py` 193/193(신규 12건 포함, 무기한 위탁 금지·append-only·cross-tenant
  forgery 방어), `backend pytest` 282/282(신규 `test_api_expert.py` 28건 — 위탁 생애주기,
  사무소 CRUD, 3중 체크 cross-tenant/cross-office 격리 포함).
...
| ✅ R3.5 | 프론트 real-mode 배선 — `DispatchQueuePage`(fire-and-forget `POST /api/v1/outbox`), 신규 `ResponseLinkPage`(`/response/:token`, 무인증; `/r/:token` 호환) | L1 | mock 모드 무변경(기존 테스트 전건), typecheck/lint 클린 |
...
| ✅ SD-4 | 런 SSE 배선 (B4b) | L3 | SEED_DESIGN §Part B5(a) | `streamCommandRun`(fetch+ReadableStream, cancel=AbortController — EventSource는 POST+Authorization 헤더를 못 써 실장 중 폐기, `plans/BACKEND_CONNECT.md` §3-2 개정) + CommandBar→RunPage 핸드오프(`liveRunStore`). `executeRun`·`RunConfig`·`useRunEngine` 반환 계약 불변, replay는 각본 유지 |
...
**SD-4 완료(2026-07-20).** `POST /api/v1/runs/stream`(B3')을 프론트에 배선했다. 상세 설계 판단은
`plans/SEED_DESIGN_2026-07-20.md` Part C 요약과 `plans/BACKEND_CONNECT.md` §3-2 개정 노트 참조 —
요약:
- **EventSource 폐기**: 설계 문서(BACKEND_CONNECT §3-2)가 전제했던 `EventSource`(`cancel=close`)는
  이 엔드포인트가 POST+`Authorization` 헤더를 요구해 애초에 쓸 수 없었다(EventSource는 GET 전용,
  커스텀 헤더·바디 불가) — `fetch()`+`ReadableStream` reader로 `event:`/`data:` 프레임을 직접
  파싱하는 `src/lib/api/runs.ts`의 `streamCommandRun()`으로 대체, `cancel()`은
  `AbortController.abort()`.
- **CommandBar→RunPage 핸드오프 설계**: `POST /runs/stream`은 조회 API가 없는 "생성+실행 겸용"
  엔드포인트라(재호출=중복 실행) RunPage가 독립적으로 스트림을 열 수 없다. `src/stores/liveRunStore.ts`
  (신규 zustand 스토어)가 CommandBar의 `startCommandRun()` 호출로 스트림을 1회만 열고, 서버가
  `run_created` 프레임으로 run_id를 알려주면 그 즉시 스토어에 항목을 만들며 promise를 resolve —
  CommandBar는 그 run_id로 `nav.toRun()`한다. RunPage는 `useLiveRunEngine(runId)`로 스토어를
  구독만 한다(컴포넌트 마운트와 무관하게 스토어가 프레임을 계속 소비하므로 재마운트해도 지금까지
  쌓인 상태를 그대로 이어 본다).
- **선택 지점**: `RunPage.tsx`가 `RUN_CONFIGS`에서 못 찾은 runId를 real 모드에서만
  `LiveRunPageContent`(→`useLiveRunEngine`)로 렌더한다. mock config 분기(`RunPageContent`)는 한
  글자도 바뀌지 않았다. `useLiveRunEngine`은 `useRunEngine`과 반환 계약(`{steps, status,
  currentIndex}`)이 완전히 동일해 `RunScreen`·`StepTimeline`은 무수정.
- **차단·완료 분기**: route 프레임이 `should_run=false`(금지어·미인식 의도)면 steps 없이 곧장
  `done`이 오는데(structured 프레임 자체가 없음), 이 경우 RunScreen의 기존 `error` 상태로
  매핑했다(reason: forbidden→`blocked`, unknown→`unknown`). `approval_required`는 mock의
  question/altLabel/승인 버튼 흐름을 그대로 재사용 — 이 런 타입은 항상 caseId가 없으므로(케이스
  생성은 Daily Briefing Rule 엔진 몫, 후속) 승인 버튼은 서버에 어떤 결정도 내리지 않고
  DonePage로만 넘긴다(mock의 "caseId 없는 커맨드 런"과 동일 원칙 — 직접 발송 없음).
- backend는 무수정(기존 구현이 계약과 완전히 일치함을 확인).
- 검증: `npm run verify`(typecheck→lint→test 617건→build) 전부 PASS(mock 모드 기존 599건 전부
  무회귀 + 신규 18건). SSE 프레임 파싱·cancel()·차단 라우팅·핸드오프(중복 POST 없음 단언 포함)를
  `global.fetch`를 실제 `ReadableStream` 바디로 목킹해 검증(`src/lib/api/runs.test.ts`·
  `src/stores/liveRunStore.test.ts`·`src/features/run/RunPage.realApi.test.tsx`·
  `src/features/briefing/CommandBar.realApi.test.tsx`). 실 backend+rag+Postgres를 띄운
  브라우저 클릭스루는 격리 워크트리 제약상 생략(태스크 지시대로 mock-fetch 통합 테스트로 대체).
