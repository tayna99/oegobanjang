...
| SD-3 | citations hydrate + briefings 슬롯 배선 (B4a) | L2 | SEED_DESIGN §Part B5(a) | `src/lib/api/citations.ts` 신설 + `citationStore.hydrate`(실패 시 mock 폴백), 브리핑 전용 스토어 슬롯 + `fetchLatestBriefing` 배선. mock 561 테스트 무변경, `npm run verify` PASS |
