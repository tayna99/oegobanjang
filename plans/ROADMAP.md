...
| ✅ 2.2 | 메시지 탭 + 스레드 대화 뷰 + M6 응답 해석(isFinal:false→확인) — 목록 원문 미노출, 원문은 스레드 내부만. 해석 확인 시 caseStore.applyInterpretationUpdates로 문서 상태 갱신(main 병합 이식, `plans/HANDOFF.md` 2026-07-16 병합 항목) | L2 | 1단계 M6, 탭별기획 §3, 블루프린트 §9-A | 해석 확인 시 상태 갱신+evidence 테스트 |
