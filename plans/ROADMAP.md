...
> **R1(목 세계 플로우 완결)은 착수하지 않았다** — 2026-07-17 사용자가 R0 완료 직후 R1을 건너뛰고
> R2(백엔드 배선)로 바로 진행하도록 명시적으로 지시했다. R1.1~1.8(회사 프로필 슬롯·온보딩→케이스
> 생성·CSV 실파싱 등, `plans/NEXT_ROADMAP_2026-07-16.md` R1 참조)은 여전히 mock 세계관 그대로다.
...
| 2.3 | 읽기 API 신설+배선 | **L3(2~3세션)** | NEXT_ROADMAP R2.3, M-6 | backend에 케이스 목록/상세·브리핑·스레드 read endpoint 추가(`db/schema.sql` 계약 준수) → caseStore/threadStore 서버 동기화(real 모드). 여기서 M-6 영속성이 해소된다 — 별도 localStorage 영속화를 만들지 말 것 |
...
되도록 교정(`src/lib/api/config.ts`). 2.3~2.6은 후속 세션에서 순차 진행한다(2.3은 NEXT_ROADMAP이
스스로 2~3세션 분량으로 예고).
