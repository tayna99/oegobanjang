...
| # | 태스크 | 분류 | 근거·비고 |
|---|---|---|---|
| 4.1 | 온보딩 O1~O5 | 블록 해제 | 목업: `외고반장 온보딩.dc.html`. DoD는 기존 표(위 M4) 그대로. |
| 4.4 | CSV 일괄 업로드 | 블록 해제 | 목업: `외고반장 CSV 업로드.dc.html`. DoD는 기존 표(위 M4) 그대로. |
| 4.5a | PC 케이스 테이블 보강(4a) | 부분 확장 | `CaseWorkbench.tsx`에 프리셋 필터 5종·담당 컬럼·서류 준비율 컬럼 추가 |
| 4.5b | PC 근로자 데이터 관리(4b) | 순신규 | 근로자 목록 + CSV 가져오기 진입점(4.4 연동) + 서류스캔 자리표시 |
| 4.5c | PC 메시지 데스크톱 분기(4c) | 부분 확장 | `useIsDesktop` 분기만 추가, M6 해석확인 로직 재사용 |
