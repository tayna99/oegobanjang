...
- 한 일: ① **AGENTS.md 재조율** — §3(현재 아키텍처)·§4(작업 전 읽을 문서)·§6(작업 범위 제한)·§10(검증 명령)을 "루트에 backend/ 없음"에서 "backend/는 legacy/backend/의 부활이 아닌 신규 구현"으로 갱신. 시작 전 `git fetch`로 확인한 결과 시작하신 다른 백그라운드 태스크(task_3b8d11b9, CLAUDE.md/README 재작성)는 아직 커밋이 없어(브랜치 `claude/trusting-tereshkova-cbfb31` 0 commits) 충돌 없이 진행. 참고로 GLOSSARY/GOTCHAS 정정 태스크(task_b834a9f7)는 `claude/charming-moser-bcd82d`에 이미 완료돼 있었음(별 브랜치, 이 PR과 무관).
  ...
  ③ **승인 decide API 구현**(`POST /api/v1/approvals/{id}/approve|reject`) — `app/domain/case_transitions.py`(src/stores/caseStore.ts CASE_TRANSITIONS 정확히 이식)·`app/domain/exceptions.py`(§5.3 게이트 8개 1:1 대응 예외)·`app/domain/pii.py`(반려 사유 PII 패턴 차단 — 등록번호/전화번호/여권번호 정규식)·`app/schemas/approval.py`·`app/services/approvals.py`(승인/반려 트랜잭션 — citation-0 잠금·본인확인 필수·high risk case는 handoff 액션만·manager는 manager_allowed+LOW severity만·멱등 replay·idempotency_key 재사용 충돌 시 409)·`app/api/v1/approvals.py`(도메인 예외→HTTP 상태 매핑, batch 엔드포인트 없음). `backend/tests/test_api_approvals.py` 12건 신규(FastAPI TestClient + `app.dependency_overrides`로 실제 마이그레이션된 임시 DB 주입).
- 남은 일 / 중단 지점: 승인 "요청" 생성 엔드포인트(`requestApproval` 대응)가 아직 없음 — 지금은 시드로만 pending 승인을 만든다. 인증/세션 없음(`decided_by_user_id`를 신뢰된 입력으로 받음). checklist(M2.6)·delegation(위임) 게이트 로직은 있으나 그걸 채우는 화면/엔드포인트는 아직 없음. 다음 자연스러운 엔드포인트: 승인 요청 생성, 케이스 목록/상세 조회.
...
- 한 일: DB_SCHEMA 설계 직후 사용자 지시로 **구현 착수 결정 9번(백엔드 형상)·10번(첫 마이그레이션 범위)을 실제로 진행** — "결정만 기록"이 아니라 스캐폴드를 실제로 만들었다. ① **`backend/`(FastAPI+SQLAlchemy 2.0+Alembic)** 신설: `app/db/base.py`(단일 DeclarativeBase, 레거시 §12-17 재발 방지) · `app/db/session.py`(SQLite `PRAGMA foreign_keys=ON`을 연결 이벤트로 강제) · `app/models/`(12파일, P1 코어 18테이블 전부 — company/user/membership/worker/citation+case_citation/document+worker_document/case+next_action/approval/evidence/run+run_step/draft+draft_variant/briefing+briefing_item, docs/DB_SCHEMA.md §4를 컬럼·CHECK·인덱스·부분 유니크까지 정확히 전사) · `migrations/versions/0001_p1_core_schema.py`(유일한 리비전 — Alembic autogenerate로 뼈대를 뽑은 뒤 트리거 2종·뷰 4종을 수기 추가) · `tests/`(pytest, create_all() 미사용 — 매 테스트 임시 SQLite에 `alembic upgrade head`를 서브프로세스로 실행해 검증, **26/26 PASS**: DDL 정합 6 + 가드레일 19 + 헬스체크 1). ② **순환 FK 실증**(`cases.prepared_run_id` ↔ `runs.id`) — SQLite는 CREATE TABLE 시점에 참조 대상 존재를 검사하지 않아 raw DDL(`db/schema.sql`)과 동일하게 동작함을 Alembic 마이그레이션으로 재확인, `Base.metadata.create_all()`은 위상정렬 실패로 못 쓴다는 것도 확인(테스트가 항상 실제 마이그레이션을 거치는 이유). ③ **SQLAlchemy 2.0 함정 발견**: `Boolean` 타입의 `create_constraint` 기본값이 2.0부터 `False`로 바뀌어 `IN (0,1)` CHECK가 자동 생성되지 않음 — 5개 Boolean 컬럼(`users.biometric_registered`/`cases.approval_required`/`next_actions.requires_approval`/`document_requirements.required`/`draft_variants.is_revised`) 전부에 `create_constraint=True, name=...` 명시해 교정. ④ **정책 결정 8건 확정**(DB_SCHEMA §13, "미결"→"결정 기록"으로 재작성): #2(`ApprovalStatus`에 `cancelled` 추가)만 코드 변경 필요 — `src/types.ts` + `src/lib/chipTone.ts`(톤은 `rejected`와 동일 `neutral`). 나머지 7건 중 법무·오너 승인이 필요한 #1(보존 정책)만 명시적으로 "제안, 미확정"으로 남기고 임의 확정하지 않음. #3~#8은 스키마가 이미 수용 가능하거나(viewer 마스킹·패키지 포맷) YAGNI로 유보(승인 정책 세분화)하거나 자연 해소(익일 처리)되거나 P3로 이연(알림 mute)되는 근거를 각각 남김.
- 남은 일 / 중단 지점: API 라우터·인증·승인 엔드포인트는 이번 스캐폴드 범위 밖(`backend/README.md` "알려진 스코프 경계" — 스키마·마이그레이션·가드레일 테스트까지가 완료 기준이었음). `drafts.thread_id`는 컬럼만 있고 FK 없음(P2 threads 도입 시 추가). P2(소통·패키지 7테이블)·P3(계정 심화·알림·에이전틱 7테이블)는 해당 마일스톤에서 이관.
- 결정 사항 (다음 세션이 알아야 할 것): ① **백엔드는 새 스캐폴드다, legacy/backend 부활이 아니다** — 레거시 결함 20건(§12)을 의도적으로 피했다(런타임 ALTER TABLE 없음, ORM relationship 실사용, 단일 Base 등). ② **테스트는 절대 `create_all()`을 쓰지 않는다** — Alembic이 유일한 스키마 생성 경로라는 원칙(§0-7)을 테스트에도 강제, `tests/conftest.py`의 `migrated_engine` 픽스처가 서브프로세스로 실제 `alembic upgrade head`를 돈다. ③ **pytest의 내장 `tmp_path`를 쓰지 않는다** — 이 개발 머신의 `%TEMP%/pytest-of-<user>/`가 2026-07-11 다른 세션에서 생긴 권한 문제로 `os.scandir` 자체가 거부되는 상태였다(공용 시스템 폴더라 삭제하지 않음, 우리 코드와 무관함을 조사로 확인). `conftest.py`는 순수 `tempfile.mkdtemp()`로 우회 — 다른 환경(CI 등)에서도 안전. ④ **AGENTS.md의 "루트에 backend/ 없음" 문구가 이 커밋으로 낡았다** — 방향 전환 시점에 이미 실행 중이던 백그라운드 문서 정리 태스크(CLAUDE.md/README를 "backend 없음"으로 재작성 중)와 정면으로 충돌하니, 다음 세션은 그 결과물과 AGENTS.md를 이 결정에 맞춰 다시 조율해야 한다(사용자에게 이미 고지·확인받음).
- verify 상태: **백엔드 PASS**(`uv run pytest`, 26/26, `backend/`에서 실행). **프론트 typecheck 0·lint 0**(`ApprovalStatus`/`chipTone.ts` 변경 무사 컴파일). **프론트 test:run — 기존 known flake 재확인**: `src/features/approvalFlow.test.tsx`의 5000ms 하드 타임아웃이 전체 스위트 동시 실행 시 간헐 발생(단독 실행 시 100% 통과, 격리 재현 완료). 원인이 이번 변경인지 확정하기 위해 `git stash`로 `types.ts`/`chipTone.ts`를 되돌린 기준 트리에서도 재현을 시도 — 그 실행은 통과했으나 표본이 작고(각 1회), 애초에 타입 레벨 변경(미사용 유니온 멤버·미사용 Record 키 추가)은 런타임 분기·타이밍에 관여할 메커니즘이 없어 논리적으로도 원인일 수 없다고 판단(이 세션 전 기록에도 동일 계열의 다른 파일 간헐 플레이크가 여러 번 남아있음 — 1.2/1.3/2.5.4 항목). `npm run verify`(전체 체인) 최초 1회는 끝까지 통과(build 도달)함도 확인됨.
- 지도/규칙 갱신: `docs/DB_SCHEMA.md` §1(백엔드 구현 상태 한 줄) · §13(전면 재작성, "미결"→"결정 기록"). `backend/README.md` 신설(세팅·마이그레이션·테스트·스코프 경계 문서화).
