...
- 남은 일 / 중단 지점: 없음(계획한 5 Phase 전부 완료). 다음 자연스러운 순서는 `GET /cases` 응답을 실제 케이스 목록 화면(현재 mockApi 기반)에 연결하는 것 — 지금은 승인 decide 루프만 연결됨.
- 결정 사항 (다음 세션이 알아야 할 것): ① B-13/B-14/B-15/B-16 번호는 세션 초반 구두로 전달된 "구현 착수 결정사항" 목록 기준이며 저장소에 커밋된 원본 파일이 없다 — 번호가 다시 언급되면 사용자에게 원문 재확인을 요청할 것(이번에 한 번 번호를 잘못 짚었었음). ② PIN은 6자리 숫자로 확정(스펙에 명시 없어 엔지니어링 결정, §13-12). ③ 계약 동기화는 수동+어댑터 매핑으로 확정 — OpenAPI codegen은 엔드포인트가 두 자릿수 중반을 넘으면 재검토(§8). ④ 이 세션의 Browser pane에서 클릭 자동화가 전혀 반응하지 않는 환경 문제가 있었다(네비게이션 링크 클릭도 무효) — 스크린샷도 타임아웃. 재현되면 curl로 프론트 어댑터와 동일한 API 시퀀스를 재현해 대체 검증할 것(이번에 그렇게 했고 완전히 성공함). ⑤ `.claude/launch.json`의 `web` 설정에 `autoPort:true`를 추가함 — 다른 세션이 5173을 점유해도 실패하지 않고 자동으로 다른 포트를 쓴다.
- verify 상태: PASS — backend pytest **125/125**, `db/validate.py --reset` **178/0**, frontend `npm run verify` **290/290**(1차 실행 중 5건 실패는 기존 문서화된 병렬 실행 간헐적 flake로 확인, 단독 재실행 7/7 + 전체 재실행 290/290으로 재확인). **실 E2E**: Docker PG 클린 시드 → uvicorn → curl로 어댑터와 동일한 요청 시퀀스(otp/request→verify→pin→GET cases→GET approvals→approve) 재현 → HTTP 200, `case_state=human_approved`, `evidence_events`에 `approval_decided`(summary='승인 완료') 기록, `d_day=24`(due 2026-08-09 - 오늘 2026-07-16) 정확히 계산됨을 DB에서 직접 확인.
- 지도/규칙 갱신: `docs/DB_SCHEMA.md`(§13-12~14, §8 계약 동기화), `backend/README.md`(전면 갱신 — 트랜잭션 경계 규약 신설, 스테일 문구 제거), `plans/ROADMAP.md`·`docs/ARCHITECTURE.md`(백엔드 접속점 게이트 정합), `legacy/missions/active/004-chroma-citations-sync.md`(신규).
