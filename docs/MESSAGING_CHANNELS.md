...
### R3 message-integrity rules (2026-07-21)

- `recipient_ref` is an audit-safe display reference only (`worker:{id}`). It is never passed to a provider as a destination. Immediately before dispatch, the adapter receives a channel-specific destination resolved from the deployment-only `WORKER_CHANNEL_RECIPIENTS` secret directory. Any environment with no destination fails closed and makes no provider request.
- The outbox row is committed while `queued`, then atomically claimed as `dispatching` and committed before `ChannelAdapter.send()` runs. If recording the provider result fails after a provider side effect, the durable `dispatching` row is not retried automatically; it must be reconciled first so an external message is never duplicated.
- Every outbound system message stores its originating `case_id`. A response-link submission uses that exact case rather than guessing from the thread's newest outbox item.
- A response token is single-use. `response_token_consumed_at` is a one-way DB transition, so duplicate POSTs return 409 and cannot create additional inbound messages, interpretations, or Evidence events.
