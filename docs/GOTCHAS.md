...
- primary 파랑(#0066FF)은 화면당 CTA 1개 + 활성 탭 + 포커스 링만
- 브리핑 카드 CTA는 정확히 2개. 3개 이상 금지 — **개정 예고(M2.6)**: Mobile.dc.html §2a 채택으로 카드 CTA는 "검토" 1개가 되고 승인 진입은 검토 깔때기(2b→2c)로만 바뀐다(블루프린트 §2). M2.6 착수 전까지는 2개 유지
...
- 같은 목적의 컴포넌트를 두 번 만들지 않는다 — 만들기 전에 `src/components/` 검색. Badge/Card/Button 변형은 variant prop으로
