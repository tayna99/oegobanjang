...
| 근거 | `Citation` | 법령·공식 안내. grade: A(법령)/B(공식)/C(통계)/E(내부)/F(합성 데이터 — 근거 사용 불가, 2026-07-11 비준. `usableCitations()`가 제외) |
...
| 반려 | `returned` | 승인 요청이 사유와 함께 되돌아온 상태. approval_pending과만 왕복(GOTCHAS §2) |
...
> 정본: `src/mocks/fixtures.ts` CASE_CARDS/CASE_SHEETS. 시나리오 배경은 docs/DESIGN_FIRST_BLUEPRINT_2026-07-11.md §3.
> 구 5인 로스터(프로토타입 v3)의 Mohammad I.·Candidate A(hiring)는 제거 — 행정사 패키지(2.4) 대상은 Batbayar E.로 이동(관제형 §2d).

| 이름 | 소속 · 국적 | 위험도 | 시나리오 |
|---|---|---|---|
| Batbayar E. | 제조2팀 · 몽골 | CRITICAL D+2 | 체류만료 경과 — high risk 행정사 검토 전용(blocked), 행정사 패키지(2.4) 대상 |
| Nguyen Van A | 제조1팀 · 베트남 | HIGH D-30 | 체류기간 연장 서류 요청, 누락 2건 — 승인 해피패스 주인공 |
| Siti R. | 포장팀 · 인도네시아 | HIGH D-3 | 고용변동 신고 기한 임박 — 신고서 초안 확인 |
| Tran Thi H. | 품질팀 · 베트남 | MEDIUM D-45 | 계약-체류 만료일 불일치 + 응답 도착 → 해석(M6) |
| Rahmat P. | 제조1팀 · 인도네시아 | MEDIUM | 필수 서류 누락(건강보험 자격득실 확인서) — 근거 수집 중 |
| Oyunaa T. | 포장팀 · 몽골 | LOW D-75 | 계약 만료 사전 모니터링 |
