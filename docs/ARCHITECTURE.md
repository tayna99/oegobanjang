...
| API 클라이언트 | `src/lib/api/`(R2.1) — `config.ts`(mock/real 전환 플래그 `API_MODE`, 기본 mock) · `client.ts`(`apiFetch` 공용 래퍼) · `auth.ts`(backend 인증 어댑터, snake_case↔camelCase 경계). `real` 모드에서만 `stores/sessionStore.ts`가 이 어댑터를 호출한다 — mock 모드(기본값)에서는 아무 화면도 이 계층을 타지 않는다 |
