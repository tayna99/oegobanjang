...
| DB 설계 계약 | `docs/DB_SCHEMA.md` — PostgreSQL 16+ 서비스 DB의 데이터 계약 정본. 실행 DDL·데모 시드·178개 회귀 검증은 `db/`(`db/schema.sql`, `db/seed_demo.sql`, `db/validate.py`)에 있고, 루트 `backend/`가 이 DDL을 Alembic 0001로 그대로 적용해 OTP 인증·승인 API를 제공한다(`backend/README.md`) — 단 프론트는 아직 호출하지 않는다 |
