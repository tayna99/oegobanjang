...
| DB 설계 계약 | `docs/DB_SCHEMA.md` — PostgreSQL 16+ 서비스 DB의 데이터 계약 정본. 실행 DDL·전역 참조 시드·데모 시드·회귀 검증(outbox·행정사 화이트라벨 포함)은 `db/`(`db/schema.sql`, `db/seed_reference.sql`, `db/seed_demo.sql`, `db/validate.py`; 로드 순서 schema→reference→demo)에 있고, 루트 `backend/`가 이 DDL을 Alembic 0001로 그대로 적용해 OTP 인증(+`GET /me`)·승인 API를 제공한다(`backend/README.md`) |
| API 클라이언트 | `src/lib/api/`(R2.1) — `config.ts`(mock/real 전환 플래그 `API_MODE`, 기본 mock) · `client.ts`(`apiFetch` 공용 래퍼) · `auth.ts`(backend 인증 어댑터, snake_case↔camelCase 경계) · `cases.ts`(R2.3 목록 + R2.4 `fetchCaseDetail` 상세 + SD-6 확장) · `briefings.ts`/`threads.ts`(R2.3, 읽기 전용 — `threads.ts`는 R3부터 응답 링크 `fetchResponseLink`/`submitResponseLink`(무인증)도 포함) · `evidence.ts`(R2.5 — `evidenceStore.append`가 real 모드에서 자동 호출) · `packages.ts`(R2.6 — `fetchPackageLink`는 무인증) · `approvals.ts`/`delegations.ts`(R2.4 — 승인 결정·요청 생성, 위임 조회) · `citations.ts`(SD-3) · `drafts.ts`(SD-5) · `outbox.ts`(R3 — `executeDispatch`, `DispatchQueuePage` real-mode 발송 실행 fire-and-forget). `real` 모드에서만 이 어댑터들이 실제로 호출된다 — mock 모드(기본값)에서는 아무 화면도 이 계층을 타지 않는다. `lib/dataSeed.ts`(`useSeedCases`/`useSeedThreads`/`useSeedThreadDetail`/`useSeedEvidence`/`useSeedCitations`/`useSeedBriefing`)가 부팅 시 seeding을 담당한다 |
| 상태 | `src/stores/` — caseStore(docUpdates 포함), approvalStore, evidenceStore, citationStore, roleStore, companyStore(briefingTime·notificationPrefs — `/settings/notifications`와 허브가 공유하는 단일 소스), threadStore(2.2 — `upsert`/`confirmInterpretation`/`receiveInbound`(근로자 응답 링크의 mock 화면에서 인바운드 메시지를 반영하고 `interpretationStatus`를 `pending_review`로만 옮긴다; real 모드는 서버의 응답 링크 API가 정규화한다). 발송 함수 없음: 승인은 `interpretationStatus`를 `confirmed`로 옮길 뿐, 실제 채널 발송은 approvalStore.dispatch 몫), sessionStore(R2.2 — real 모드 전용 세션. localStorage에 세션 토큰만 영속화하고 부팅 시 복원, 성공 시 roleStore를 세션 멤버십으로 갱신) |
...
| 대상 | 위치 | 이유 |
|---|---|---|
| `RUN_CONFIGS` / replay 런(#4788 재생) | `src/mocks/runs.ts` | replay·오프라인 데모의 정본. real 모드 커맨드 런은 `POST /runs/stream`(SSE, `src/lib/api/runs.ts`)으로 별도 배선됐지만(SD-4 완료), replay는 서버 대체 대상이 아니다(`plans/BACKEND_CONNECT.md` §3-2 "RUN_CONFIGS는 삭제하지 않는다") |
...
> 반대로 "API는 이미 있는데 프론트가 아직 안 부르던 것"(`GET /citations`·`GET /briefings/latest`·
> `POST /runs/stream`)은 영구 mock이 **아니다** — SD-3·SD-4에서 배선 완료됐다. 구분은 시드 설계
> 문서 `plans/SEED_DESIGN_2026-07-20.md` Part A/B와 ROADMAP의 SD 트랙 표를 정본으로 한다.
