...
12. **승인 PIN 정책·checklist 동반 제출 — 결정(엔지니어링 재량, PR #12 코드 리뷰로 강화).** PIN은 6자리 숫자(`^\d{6}$`) — 어떤 스펙 문서에도 길이·형식이 명시돼 있지 않아 확정했다. 원문은 저장하지 않고 `users.pin_hash`에 `hash_secret`(HMAC-SHA256, §13-11과 동일 pepper)만 남긴다. `POST /api/v1/auth/pin`으로 등록·변경하되 **세션만으로는 안 된다** — 방금 새로 발급받은 OTP(`otp_code`)로 전화 소지를 재확인해야 한다(로그인용 OTP는 `/otp/verify`에서 이미 소비되므로 별도로 `/otp/request`를 한 번 더 호출). 세션 탈취자가 재확인 없이 PIN을 덮어써 본인확인 게이트 전체를 우회하는 경로를 막기 위한 결정이다(코드 리뷰 P1-2). `identity_method='biometric'`은 **승인 API가 아직 받지 않는다** — `users.biometric_registered`는 등록 여부일 뿐 실제 생체 서명 증명이 아니라서, 세션만 있으면 누구나 'biometric'이라고 주장해 통과시킬 수 있었다(코드 리뷰 P1-3). WebAuthn 등 실서명 검증이 붙기 전까지는 `pin`만 유효한 `identity_method`이며, `biometric`을 제출하면 422로 거부한다 — 컬럼·CHECK 자체는 미래를 위해 그대로 둔다. checklist는 전용 PATCH 엔드포인트를 두지 않는다 — `trg_approvals_update_guard`가 pending 승인의 모든 UPDATE에 status 전이(approved/rejected)를 요구하므로, checklist 갱신은 decide 요청에 동반 제출해 결정 UPDATE와 같은 트랜잭션에서 key 병합 처리한다.
...
15. **CORS — 결정(엔지니어링 재량, PR #12 코드 리뷰).** 프론트(`src/lib/api.ts`)가 백엔드와 다른 origin에서 JSON 본문 + Bearer 토큰으로 호출하면 브라우저가 preflight(OPTIONS)를 먼저 보낸다. `CORSMiddleware`를 추가하되 origin 허용 범위는 `auth_pepper`(§13-11)와 같은 fail-safe 원칙을 따른다 — `environment=local`이면 `localhost`/`127.0.0.1`(임의 포트) 정규식을 기본 허용하고, 그 외 환경은 명시적으로 `CORS_ALLOW_ORIGIN_REGEX`를 설정하기 전까지 CORS를 아예 붙이지 않는다(전면 차단이 안전 기본값). 쿠키를 쓰지 않으므로 `allow_credentials=False`로 두어 origin 매칭을 단순하게 유지한다.
