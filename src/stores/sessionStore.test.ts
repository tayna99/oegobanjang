...
describe('sessionStore (R2.2 — 실서버 세션, API_MODE=real일 때만 쓰인다)', () => {
  ...
  it('logout은 세션을 초기화하고 roleStore를 viewer로 fail-close하며 백엔드에 폐기를 요청한다', async () => {
    ...
    // 코드리뷰 회귀(PR #19 P1): roleStore.reset()(mock 데모 기본값 manager)이 아니라
    // setRole('viewer')여야 한다 — 그렇지 않으면 로그아웃 직후 같은 SPA 세션에서
    // 관리자 권한 UI가 다시 열린다(fail-open).
    expect(useRoleStore.getState().role).toBe('viewer');
    expect(window.localStorage.getItem('oegobanjang-session-token')).toBeNull();
