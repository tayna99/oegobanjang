...
describe('lib/api/packages', () => {
  ...
  it('issuePackageLink는 POST /api/v1/packages/{caseId}/link를 호출한다', async () => {
    const mockFetch = vi.fn().mockResolvedValue(new Response(JSON.stringify(dto), { status: 201 }));
    global.fetch = mockFetch as unknown as typeof fetch;

    const result = await issuePackageLink('batbayar');

    expect(mockFetch).toHaveBeenCalledWith(
      'http://localhost:8000/api/v1/packages/batbayar/link',
      expect.objectContaining({ method: 'POST' }),
    );
    expect(result).toEqual({ caseId: 'batbayar', issuedAt: dto.issued_at, expiresAt: dto.expires_at });
  });

  it('fetchPackageLink는 GET /api/v1/packages/{caseId}/link를 호출하고 Authorization 헤더가 없다', async () => {
    const mockFetch = vi.fn().mockResolvedValue(new Response(JSON.stringify(dto), { status: 200 }));
    global.fetch = mockFetch as unknown as typeof fetch;

    const result = await fetchPackageLink('batbayar');

    expect(mockFetch).toHaveBeenCalledWith('http://localhost:8000/api/v1/packages/batbayar/link', expect.anything());
    const headers = mockFetch.mock.calls[0][1]?.headers as Record<string, string>;
    expect(headers.Authorization).toBeUndefined();
    expect(result.caseId).toBe('batbayar');
  });
  ...
  it('링크가 만료·미발급이면 404 ApiError를 던진다', async () => {
    ...
    await expect(fetchPackageLink('no-such-case')).rejects.toBeInstanceOf(ApiError);
  });
});
