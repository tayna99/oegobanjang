...
// 우선순위(탭별기획 §3.2): 응답 도착 > 승인 대기 > 발송됨 > 초안. '확인 완료'는 응답 해석을
// 확인한 뒤(interpretationStatus:'confirmed')의 후속 상태.
// 톤은 chipTone.ts의 기존 매핑을 그대로 따른다(NEXT_ROADMAP D-2, v1 BadgeTone 폐기) —
// 응답 도착·승인 대기는 approvalStatusTone('pending')과 같은 'approval'(블루, rules/design.md
// §5 "승인 필요(정보)"가 같은 표에 있다), 발송됨은 'positive'(초록, "완료").
export function threadBadge(thread: MessageThread): ThreadBadge {
  if (thread.interpretationStatus === 'pending_review') {
    return { label: '응답 도착', tone: 'approval' };
  }

  if (thread.draftCaseId && thread.messages.length === 0) {
    return { label: '승인 대기', tone: 'approval' };
  }
  ...
  if (lastOutStatus === 'sent') {
    return { label: '발송됨', tone: 'positive' };
  }
